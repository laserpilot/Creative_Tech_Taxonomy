...
export function showModal(nodeData) {
  ...
  const isInteractionTaxonomy =
    nodeData.data.cost !== undefined || nodeData.data.accuracy !== undefined || nodeData.data.latency !== undefined
  ...
  if (isInteractionTaxonomy) {
    ...
    const useCases =
      nodeData.data.use_cases && nodeData.data.use_cases.length > 0
    ...
    if (technicalDetails.length > 0 || useCases.length > 0) {
      additionalFields = `
        <div class="modal-section interaction-details">
          ...
          ${technicalDetails
            .map((detail) => `<div class="detail-item"><strong>${detail.label}:</strong> ${detail.value}</div>`)
            .join("")}
          ${
            useCases.length > 0
    ...
    ${
      validLinks.length > 0
        ? `<div class="modal-section">
         ...
         <ul class="links-list">
           ${validLinks
             .map(([type, url]) => `<li><strong>${type}:</strong> <a href="${url}" target="_blank">${url}</a></li>`)
             .join("")}
         </ul>
       </div>`
        : ""
